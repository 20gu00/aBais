...
// 获取pvc列表，支持过滤、排序、分页
func GetPvcs(ctx *gin.Context) {
	// 1.参数
	params := new(param.GetPvcInput)
	if err := ctx.Bind(params); err != nil {
		zap.L().Error("C-GetPvcs 绑定请求参数失败", zap.Error(err))
	...
	if err != nil {
		zap.L().Error("C-GetPvcs 获取k8s的client失败", zap.Error(err))
	...
	// 3.resp
	response.RespOK(ctx, "获取Pvc列表成功", data)
}
