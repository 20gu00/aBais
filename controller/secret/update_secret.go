...
func UpdateSecret(ctx *gin.Context) {
	// 1.参数
	params := new(param.UpdateSecretInput)
	// PUT ShouldBindJSON
	if err := ctx.ShouldBindJSON(params); err != nil {
		zap.L().Error("C-UpdateSecret 绑定请求参数失败", zap.Error(err))
	...
	if err != nil {
		zap.L().Error("C-UpdateSecret 获取k8s的client失败", zap.Error(err))
	...
	// 3.resp
	response.RespOK(ctx, "更新Secret成功", nil)
}
